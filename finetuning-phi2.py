...
def summarize_article(index, dataset, model, tokenizer):
    ...
    print(f"Input token length: {summary_length}")

    formatted_prompt = f"Instruct: Summarize the following article in concise sentences.\nArticle:\n{article}.\nOutput:\n"
    #formatted_prompt = f"Instruct: Summarize the following article in concise sentences.\n{article}\nOutput:\n"

    pad_token_id = tokenizer.eos_token_id if tokenizer.eos_token_id is not None else tokenizer.pad_token_id
    res = gen(model,formatted_prompt,summary_length,pad_token_id=pad_token_id)
    #print(res[0])
...
# def get_max_length(model):
#     conf = model.config
#     max_length = getattr(conf, "n_positions", None) or getattr(conf, "max_position_embeddings", None)
#     if max_length:
#         print(f"Found max length: {max_length}")
#         return max_length

#     print("Max length not found in model config. Using default.")
#     return 1024  # Default max length for many models
...
def preprocess_dataset(tokenizer: AutoTokenizer, max_length: int,seed, dataset):
    ...
    max_length = max_length or get_max_length(tokenizer.model_max_length)
...
############### LOADING DATASETS #############
print(f"{str(int(time.time()))} - Phi2-Finetuning")
...
peft_training_args = TrainingArguments(
     ...
     learning_rate=5e-5,
     logging_steps=500,
     eval_steps=100,#1000 is too much for evaluation, need to be adjusted to 5000
     save_steps=100, 
     max_steps=1000,
     warmup_ratio=0.1,
...
peft_trainer.save_model("/scratch/dsu.local/khanh.nguyen/working/peft-phi-2-model-1000")
