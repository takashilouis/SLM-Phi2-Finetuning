...
def generate_summary(model, tokenizer, article, max_new_tokens=256):
    messages = create_chat_prompt(article)
...
print("Jan15-2pm22! Phi-3-mini-4k-instruct")
...
pad_token_id = tokenizer.eos_token_id if tokenizer.eos_token_id is not None else tokenizer.pad_token_id
res = gen(original_model,formatted_prompt,summary_length,pad_token_id=pad_token_id)
#print(res[0])
output = res[0].split('Output:\n')[1]
